...
setwd("[Add working directory here]/Correlations/Analysis/Theta_S/Approx_LR_2000")

library(readr)

folder <- "[Add root directory here]/Correlations/Analysis/Theta_S/"       # path to folder that holds multiple .csv files
...
# This will create a list of values from 0 to 30000 every 2000, which will be use as standard time intervals
# Transform the Temperature of the Antarctic by linear appoximation

ValuesTime <- seq(0, 30000, 2000)
...
filenameAllData="Southern_AllApprox_LR_2000.csv"

for (i in 1:length(DFs_LR))
{

  filename<-paste0(names(DFs_LR)[i], "_2000.csv")
